...
// ../../.wrangler/tmp/pages-0FqYw1/functionsWorker-0.3076695122545978.mjs
...
async function onRequestPost({ request, env }) {
  ...
  const question = clean(body.question, 3e3);
  ...
  const payload = {
    ...
    body: `A new question was submitted on the zuhd.au FAQ page.
...
async function onRequestOptions2() {
  return new Response(null, { status: 204, headers: JSON_HEADERS2 });
}
...
async function onRequestPost2({ request, env }) {
  ...
  if (!Number.isFinite(amount) || amount < 1 || amount > 1e5) {
    return json2({ error: "Amount must be between 1 and 100,000." }, 400);
  }
  if (!env.SHOPIFY_ADMIN_TOKEN) {
    return json2({ error: "Donations are temporarily unavailable. Please contact info@zuhd.au." }, 503);
  }
  const store = env.SHOPIFY_STORE || "zuhd-store.myshopify.com";
  const cause = typeof body.cause === "string" && body.cause.trim() ? body.cause.trim().slice(0, 60) : "";
  const draftOrder = {
    draft_order: {
      line_items: [
        {
  ...
  if (!res.ok) {
    ...
    return json2({ error: "Could not start the donation checkout. Please try again later." }, 502);
  }
  const data = await res.json();
  const invoiceUrl = data?.draft_order?.invoice_url;
  if (!invoiceUrl) {
    return json2({ error: "Checkout link missing from Shopify response." }, 502);
  }
  return json2({ payment_url: invoiceUrl });
}
...
// .wrangler/tmp/bundle-8sKXwR/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-8sKXwR/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.3076695122545978.js.map
